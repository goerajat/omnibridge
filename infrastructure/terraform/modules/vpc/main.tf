...
# Admin port (8083) - from monitoring SG for OmniView proxy
resource "aws_vpc_security_group_ingress_rule" "persistence_admin_from_monitoring" {
  security_group_id            = aws_security_group.persistence.id
  description                  = "Admin port from monitoring for OmniView"
  from_port                    = 8083
  to_port                      = 8083
  ip_protocol                  = "tcp"
  referenced_security_group_id = aws_security_group.monitoring.id
