@echo off
REM Run the Sample FIX Initiator (Trading Client)
REM Usage: run-initiator.bat [options]
REM Options:
REM   -h, --host <host>       Target host (default: localhost)
REM   -p, --port <port>       Target port (default: 9876)
REM   -s, --sender <id>       SenderCompID (default: CLIENT)
REM   -t, --target <id>       TargetCompID (default: EXCHANGE)
REM   --heartbeat <seconds>   Heartbeat interval (default: 30)
REM   --persistence <dir>     Persistence directory
REM   --auto                  Auto-send sample orders
REM   --count <n>             Number of orders in auto mode (default: 10)
