...
start "%ACCEPTOR_TITLE%" /min cmd /c "java %JVM_OPTS% -cp "%CP%" com.fixengine.samples.acceptor.SampleAcceptor -c latency-acceptor.conf --latency --fill-rate 1.0 > "%ACCEPTOR_LOG%" 2>&1"
...
java %JVM_OPTS% -cp "%CP%" com.fixengine.samples.initiator.SampleInitiator -c latency-initiator.conf --latency %* 2>&1 | powershell -Command "$input | Tee-Object -FilePath '%INITIATOR_LOG%'"
