...
start "" /b java %JVM_OPTS% %CHRONICLE_OPTS% -jar "%UBER_JAR%" --latency > ouch-acceptor.log 2>&1
...
echo.
java %JVM_OPTS% %CHRONICLE_OPTS% -cp "%UBER_JAR%" com.omnibridge.apps.ouch.initiator.SampleOuchInitiator --latency --warmup-orders %WARMUP_ORDERS% --test-orders %TEST_ORDERS% --rate %RATE%
